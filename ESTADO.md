...
1. **F5 publicación: COMPLETA (13-07-2026).** Repo privado en **https://github.com/gerobena/geosurrogate**; primer push con 22 commits; **CI en verde a la primera** (run 29287998202: 2m08s, 49/49 tests en Ubuntu, e2e con deepgp 1.2.1 real, cero skips — validado que `GEOSURROGATE_RSCRIPT=Rscript` hace el e2e portable). Commits vinculados al perfil `gerobena` (identidad noreply verificada vía API). `gh` 2.96.0 instalado (`C:\Program Files\GitHub CLI\gh.exe`) y autenticado (keyring). Restos opcionales: capturas/GIF del dashboard en vivo para el README; borrar la rama local `backup-before-identity-rewrite` cuando el autor confirme; aviso benigno del CI: actions con Node 20 deprecado (actualizar `actions/*` cuando toque).
